...
2. The TurboVNC Viewer's `LocalUsernameLC` parameter now affects the SSH
username if the SSH username is unspecified.
...
19. Fixed a denial-of-service (DoS) vulnerability in the TurboVNC Server,
...
25. Fixed an issue in the TurboVNC Viewer's built-in SSH client whereby the SSH
...
33. The simple web server for noVNC (part of the TurboVNC Server) now supports
