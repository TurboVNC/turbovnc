...
int proxy_http_negotiate (Proxy_Socket p, int change)
{
	...
	if (p->cfg.proxy_username[0] || p->cfg.proxy_password[0]) {
	    ...
	    for (i = 0, j = (int)strlen(buf2); i < len; i += 3, j += 4)
...
int proxy_socks4_negotiate (Proxy_Socket p, int change)
{
	...
	length = (int)strlen(p->cfg.proxy_username) + namelen + 9;
...
int proxy_socks5_negotiate (Proxy_Socket p, int change)
{
	...
	if (p->state == 5) {
		...
		plen = (int)strlen(p->cfg.proxy_password);
...
#define ENSURE(n) do { \
	...
	if (cfg->proxy_telnet_command[so] == '\\') {
		...
		eo += 4;
			      ...
			      "user", 4) == 0) {
		int userlen = (int)strlen(cfg->proxy_username);
		ENSURE(userlen);
		...
		eo += 4;
			      ...
			      "pass", 4) == 0) {
		int passlen = (int)strlen(cfg->proxy_password);
		ENSURE(passlen);
		...
		eo += 4;
			      ...
			      "proxyhost", 9) == 0) {
		int phlen = (int)strlen(cfg->proxy_host);
		ENSURE(phlen);
