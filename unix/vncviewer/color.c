...
static int nBGR233ColorsAllocated;
...
static void AllocateExactBGR233Colors();
static Bool AllocateBGR233Color(int r, int g, int b);
 ...
 * check if there's a visual with the same depth as the RFB server, and failing
 * that, we try to use an 8-bit visual.
 *
 * If forceTrueColor is true, then we try to use a TrueColor visual - if
 * requestedDepth is set, then the visual must be of that depth.  Otherwise,
 * any TrueColor visual, regardless of depth, may be used.
 *
 * If neither forceOwnCmap nor forceTrueColor is specified, we use the X
 * server's default visual and colormap.  If this is TrueColor, then we just
 * ask the RFB server for this format.  If the default isn't TrueColor, or if
 * useBGR233 is specified, then we ask the RFB server for a BGR233 pixel format
 * and use a lookup table to translate to the nearest colors provided by the
 * X server.
 */
...
/*
 * GetPseudoColorVisualAndCmap() tries to find a PseudoColor visual of the
 * given depth.  If successful, it sets vis, visdepth, cmap, and myFormat and
 * also sets the appropriate resources on the toplevel widget.
 */
 ...
 *
 * It calls AllocateExactBGR233Colors() to allocate some exact BGR233 colors
 * (limited by space in the colormap and/or by the value of the nColors
 * resource).  If the number allocated is less than BGR233_SIZE, then it fills
 * the rest in using the "nearest" colors available.  How this is done depends
 * on the value of the useSharedColors resource.  If it's false, then we use
 * only colors from the exact BGR233 colors we've just allocated.  If it's
 * true, then we also use other clients' "shared" colors available in the
 * colormap.
 */
...
static void SetupBGR233Map()
{
  ...
  if (visdepth > 8)
    appData.nColors = 256; /* ignore nColors setting for > 8-bit depth */

  for (i = 0; i < BGR233_SIZE; i++) {
    BGR233ToPixel[i] = INVALID_PIXEL;
  }

  AllocateExactBGR233Colors();

  fprintf(stderr, "Got %d exact BGR233 colors out of %d\n",
          nBGR233ColorsAllocated, appData.nColors);

  if (nBGR233ColorsAllocated < BGR233_SIZE) {

    if (visdepth > 8) { /* shouldn't get here */
      fprintf(stderr, "Error: couldn't allocate BGR233 colors even though "
              "depth is %d\n", visdepth);
      exit(1);
    }
    ...
    if (appData.useSharedColors) {
      ...
      /* Try to find existing shared colors.  This is harder than it sounds,
         ...
         private, or unallocated.  What we do is go through the colormap and,
         for each entry, try to exactly allocate its RGB values.  If this
         returns a different color value, then the color is definitely either
         private or unallocated and thus of no use to us.  If we get back the
         same color value again, then it's likely that it's a shared color.
         However, it is possible that it was actually an unallocated color that
         we've now allocated.  We minimize this possibility by going through
         the colors in reverse order - this helps, becuse the X server
         allocates new colors from the lowest number up, so it should only be a
         problem for the lowest unallocated color.  Got that? */
...
/*
 * AllocateExactBGR233Colors() attempts to allocate each of the colors in the
 * BGR233 color cube, stopping when an allocation fails.  The order it does
 * this in is such that we should get a fairly well-spread subset of the cube,
 * regardless of how many allocations are made.  There's probably a neater
 * algorithm for doing this, but it's not obvious to me anyway.  The way this
 * algorithm works is:
 *
...
static void AllocateExactBGR233Colors()
{
  ...
  while (1) {
    ...
    for (gi = 0; gi < gn; gi++) {
      for (bi = 0; bi < bn; bi++) {
        if (!AllocateBGR233Color(rv[ri], gv[gi], bv[bi]))
          return;
      }
    }
    rn++;
...
/*
 * AllocateBGR233Color() attempts to allocate the given BGR233 color as a
 ...
 * allocation fails or when we would exceed the number of colors specified in
 * the nColors resource.
 */
...
static Bool AllocateBGR233Color(int r, int g, int b)
{
  ...
  if (nBGR233ColorsAllocated >= appData.nColors)
    return False;
