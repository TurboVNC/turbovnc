...
rfbSslCtx *rfbssl_init(rfbClientPtr cl, Bool anon)
{
    ...
    if ((ret = gnutls_init(&ctx->session, GNUTLS_SERVER)) !=
        GNUTLS_E_SUCCESS) {
    ...
    if ((ret = gnutls_set_default_priority(ctx->session)) !=
        GNUTLS_E_SUCCESS) {
    ...
    if (anon) {
        ...
        if ((ret = gnutls_certificate_set_x509_key_file(ctx->x509_cred,
            rfbAuthX509Cert, keyfile, GNUTLS_X509_FMT_PEM)) !=
            GNUTLS_E_SUCCESS) {
