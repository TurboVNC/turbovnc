                                       ...
                                       int h)
{
  ...
  if (zlibBeforeBufSize < maxRawSize) {
    ...
    if (zlibBeforeBuf == NULL)
      ...
      zlibBeforeBuf = (char *)rfbRealloc(zlibBeforeBuf, zlibBeforeBufSize);
  }
  ...
  (*cl->translateFn) (cl->translateLookupTable, &rfbServerFormat, &cl->format,
                      fbptr, zlibBeforeBuf, rfbFB.paddedWidthInBytes, w, h);
  ...
  cl->rfbBytesSent[rfbEncodingZlib] +=
    (sz_rfbFramebufferUpdateRectHeader + sz_rfbZlibHeader + zlibAfterBufLen);
...
Bool rfbSendRectEncodingZlib(rfbClientPtr cl, int x, int y, int w, int h)
{
  ...
  while (linesRemaining > 0) {
     ...
     */
    if ((ublen > 0) && (linesToComp == maxLines)) {
      if (!rfbSendUpdateBuf(cl))
        return FALSE;
    }
